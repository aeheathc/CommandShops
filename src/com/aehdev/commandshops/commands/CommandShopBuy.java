...
public class CommandShopBuy extends Command {
    ...
    public CommandShopBuy(CommandShops plugin, String commandLabel, CommandSender sender, String command) {
    ...
    private boolean shopBuy(Shop shop, ItemInfo item, int amount) {
        ...
        if (!isShopController(shop)) {
            if (!pData.payPlayer(player.getName(), shop.getOwner(), totalCost)) {
                ...
                if (!pData.payPlayer(player.getName(), shop.getOwner(), totalCost)) {
                    player.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "Unexpected money problem: could not complete sale.");
                    return true;
                }
            }
        }
