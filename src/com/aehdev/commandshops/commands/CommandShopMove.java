...
public class CommandShopMove extends Command {
    ...
    public CommandShopMove(CommandShops plugin, String commandLabel, CommandSender sender, String command) {
    ...
    public boolean process() {
        ...
        if(!(sender instanceof Player)) {
            sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "Console is not implemented yet.");
            return false;
        }
        ...
        if (matcher.find()) {
            ...
            if(thisShop == null) {
                sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "Could not find shop: " + ChatColor.WHITE + id);
                return false;
            }

            // check if player has access
            if (!thisShop.getOwner().equalsIgnoreCase(player.getName())) {
                player.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "You must be the shop owner to move this shop.");
                return false;
            }
            ...
            if (shopPositionOk(xyzA, xyzB, player.getWorld().getName())) {
                ...
                if (Config.SHOP_CHARGE_MOVE) {
                    if (!canUseCommand(CommandTypes.MOVE_FREE)) {
                        if (!plugin.getPlayerData().get(player.getName()).chargePlayer(player.getName(), Config.SHOP_CHARGE_MOVE_COST)) {
                            ...
                            player.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "You need " + plugin.getEconManager().format(Config.SHOP_CHARGE_MOVE_COST) + " to move a shop.");
                            return false;
                        }
                    }
                }
                ...
                if (plugin.getShopData().saveShop(thisShop)) {
                    player.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.WHITE + shopName + ChatColor.DARK_AQUA + " was moved successfully.");
                    return true;
                } else {
                    player.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "There was an error, could not move shop.");
                    return false;
                }
            } else {
                // insert the old cuboid back into the world
