...
public class CommandShopSet extends Command {
    ...
    public CommandShopSet(CommandShops plugin, String commandLabel, CommandSender sender, String command) {
    ...
    private boolean shopSetUnlimited() {
        ...
        if (sender instanceof Player) {
            ...
            if (!canUseCommand(CommandTypes.ADMIN)) {
                player.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "You must be a shop admin to do this.");
                return true;
            }            
        } else {
            sender.sendMessage("Console is not implemented yet.");
        ...
        if (matcher.find()) {
            ...
            sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "Unlimited money was set to " + ChatColor.WHITE + shop.isUnlimitedMoney());
            plugin.getShopData().saveShop(shop);
            return true;
        }
        ...
        if (matcher.find()) {
            ...
            sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "Unlimited stock was set to " + ChatColor.WHITE + shop.isUnlimitedStock());
            plugin.getShopData().saveShop(shop);
            return true;
        }
    ...
    private boolean shopSetNotification() {
        ...
        if(sender instanceof Player) {
            ...
            // Check if Player can Modify
            if (!shop.getOwner().equalsIgnoreCase(player.getName())) {
                sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "You must be the shop owner to set this.");
    ...
    private boolean shopSetOwner() {
        ...
        if (sender instanceof Player) {
            ...
            if(!canUseCommand(CommandTypes.ADMIN)) {
                sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + shop.getName() + " is no longer buying items.");
                reset = true;
            }
        } else {
            sender.sendMessage("Console is not implemented yet.");
        ...
        if (matcher.find()) {
            ...
            if (!canUseCommand(CommandTypes.SET_OWNER)) {
                sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "You do not have permission to do this.");
                return true;
            }  else if ( !canCreateShop(name) ) {
                sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "that player already has the maximum number of shops!");
                return true;
            } else {
                shop.setOwner(name);
                ...
                notifyPlayers(shop, new String[] { CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + shop.getName() + " is now under new management!  The new owner is " + ChatColor.WHITE + shop.getOwner() } );
                return true;
            }
        }
    ...
    private boolean shopSetName() {
        ...
        if (matcher.find()) {
            ...
            shop.setName(name);
            plugin.getShopData().saveShop(shop);
            notifyPlayers(shop, new String[] { CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "Shop name is now " + ChatColor.WHITE + shop.getName() } );
            return true;
        }
    ...
    private boolean shopSetHelp() {
        ...
        sender.sendMessage(CommandShops.CHAT_PREFIX + ChatColor.DARK_AQUA + "The following set commands are available: ");
        sender.sendMessage("   " + "/" + commandLabel + " set buy [item name] [price] <bundle size>");
        sender.sendMessage("   " + "/" + commandLabel + " set sell [item name] [price] <bundle size>");
        sender.sendMessage("   " + "/" + commandLabel + " set max [item name] [max number]");
        sender.sendMessage("   " + "/" + commandLabel + " set manager +[playername] -[playername2]");
