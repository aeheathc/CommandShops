...
public class CommandShopBrowse extends Command {
    ...
    public CommandShopBrowse(CommandShops plugin, String commandLabel, CommandSender sender, String command) {
     ...
     * 
     * @param shop the shop
     * @param searchName Player's input item name to search for
     */
    private void printInventoryItem(Shop shop, String searchName)
    {
        ...
        if(itemn == null)
        {
        	sender.sendMessage(String.format("No item found for %s", searchName));
        	return;
        }
        InventoryItem item = items.get(itemn.name);
        if(item == null)
        {
        	sender.sendMessage(String.format("%s does not have %s", inShopName,itemn.name));
        	return;
        }
        ...
        String message = ChatColor.DARK_AQUA + "The shop " + ChatColor.WHITE + inShopName
        				+ ChatColor.DARK_AQUA + " has " + ChatColor.WHITE + itemn.name + ':';
        sender.sendMessage(message);
        message = ChatColor.DARK_AQUA + "[" + ChatColor.WHITE + "Stock: "
       				+ (shop.isUnlimitedStock() ? "Inf." : item.getStock())
       				+ ChatColor.DARK_AQUA + "]"
       				+ (maxstock > 0
       						? (" [" + ChatColor.WHITE + "Max Stock: " + maxstock + ChatColor.DARK_AQUA + "]")
       						: "");
        sender.sendMessage(message);
        message = ChatColor.GOLD + "Selling: " + ChatColor.DARK_AQUA + " ["
        			+ ChatColor.WHITE + sellPrice + ChatColor.DARK_AQUA + "]"
        			+ (sellbundle > 1 ?
        				(" [" + ChatColor.WHITE + "Bundle: " + sellbundle + ChatColor.DARK_AQUA + "]")
        				: "");
        sender.sendMessage(message);
        message = ChatColor.GREEN + "Buying: " + ChatColor.DARK_AQUA + " ["
    				+ ChatColor.WHITE + buyPrice + ChatColor.DARK_AQUA + "]"
    				+ (buybundle > 1 ?
    					(" [" + ChatColor.WHITE + "Bundle: " + buybundle + ChatColor.DARK_AQUA + "]")
    					: "");
        sender.sendMessage(message);
    }
